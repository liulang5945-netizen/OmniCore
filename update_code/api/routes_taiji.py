...
def feed_status():
    """获取喂养引擎状态"""
    try:
        from taiji.feed_engine import get_feed_engine
...
def feed_text(text: str, source: str = "manual", category: str = "knowledge"):
    """直接喂态极一段文字"""
    try:
        from taiji.feed_engine import get_feed_engine
        engine = get_feed_engine()
...
def feed_file(file_path: str, category: str = "knowledge"):
    """喂态极吃一个文件"""
    try:
        from taiji.feed_engine import get_feed_engine
        engine = get_feed_engine()
        item = engine.feed_file(file_path=file_path, category=category)
        if item:
            return {
                "status": "ok",
                "quality_score": item.quality_score,
...
def feed_directory(dir_path: str, category: str = "code"):
    """喂态极吃一个目录下的所有文件"""
    try:
        from taiji.feed_engine import get_feed_engine
        engine = get_feed_engine()
        count = engine.feed_directory(dir_path=dir_path, category=category)
...
def sleep_status():
    """获取睡眠引擎状态"""
    try:
        from taiji.sleep_engine import get_sleep_engine
...
def play_status():
    """获取玩耍引擎状态"""
    try:
        from taiji.play_engine import get_play_engine
...
def life_status():
    """获取生命状态（需求、状态、心跳数）"""
    try:
        from taiji.life_scheduler import get_life_scheduler
...
def _load_dataset_files(file_names: list, _safe_put=None) -> tuple:
    ...
    for idx, fname in enumerate(file_names):
        ...
        if not os.path.exists(fpath):
            logger.warning(f"数据集文件不存在: {fpath}")
...
def _load_with_file_parser(fpath: str, ext: str, conv_data: list, _safe_put=None):
    ...
    from tools.file_parser import parse_file_to_text, IMAGE_EXTENSIONS
    ...
    async def event_generator():
        ...
        def train_worker():
            ...
            try:
                ...
                from taiji.trainer import ModelSelfTrainer, build_dataset
                ...
                try:
                    from taiji.data_cleaner import clean_training_data
                ...
                # 微调保存到 model_name 的父目录，确保重启后自动加载微调后的权重
                model_path = getattr(app_state, "_loaded_model_name", "") or ""
                if model_path and os.path.isdir(model_path):
                    # model_name 指向 best/ 或某个 step/，保存到其父目录
                    save_dir = os.path.dirname(model_path)
                else:
                    save_dir = get_external_path(
                        os.path.join("taiji_checkpoints", "finetune")
                    )
