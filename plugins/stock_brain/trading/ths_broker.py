...
同花顺交易适配器
================
通过 easytrader 控制同花顺客户端自动下单。
需要：1) 同花顺客户端运行并登录  2) pip install easytrader
"""
import logging
...
logger = logging.getLogger("StockBrain.THSBroker")


class THSBroker(BrokerAdapter):
    """同花顺交易适配器（easytrader）"""

    def __init__(self, account_path: str = ""):
        """
        Args:
            account_path: 同花顺账号配置文件路径（easytrader 生成的 JSON）
        """
        self._user = None
    ...
    def connect(self, **kwargs) -> bool:
        try:
            ...
            if self._account_path:
                self._user.connect(self._account_path)
            else:
                self._user.connect(r"C:\同花顺软件\同花顺\xiadan.exe")
    ...
    def _check_connected(self):
        if not self.is_connected():
            raise RuntimeError("同花顺未连接，请先调用 connect()")


# 注册到工厂
register_broker("ths", THSBroker)